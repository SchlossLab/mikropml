...
You can read the [`caret::preProcess`](https://rdrr.io/cran/caret/man/preProcess.html) documentation for more information. If you want to include them, you can use the arguments `remove_nzv=FALSE` and `remove_zv=FALSE`. For this to work, you cannot collapse correlated features (otherwise it errors out because of the underlying `caret` function we use). 
...
If you want to be more nuanced in how you remove near-zero variance features (e.g. change the default 10% cutoff for the percentage of distinct values out of the total number of samples), you can use the `caret::preProcess` function after running `preprocess_data` with `remove_nzv=FALSE` (see the [`caret::nearZeroVar`](https://rdrr.io/cran/caret/man/nearZeroVar.html) function for more information).
