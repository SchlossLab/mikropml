---
title: "Preprocessing data prior to machine learning"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Preprocessing data prior to machine learning}
...
You can also normalize the data in different ways. You can implement any method used by the `caret::preProcess()` function (the `method` argument). Note that these methods are only applied to continuous variables. 
...
You can read the `caret::preProcess()` documentation for more information. If you want to include them, you can use the arguments `remove_nzv=FALSE` and `remove_zv=FALSE`. For this to work, you cannot collapse correlated features (otherwise it errors out because of the underlying `caret` function we use). 

```{r}
# don't remove features with near-zero variance
preprocess_data(dataset = nonvar_df, outcome_colname = "outcome", remove_nzv = FALSE, remove_zv = FALSE, collapse_corr_feats = FALSE)
```


If you want to be more nuanced in how you remove near-zero variance features (e.g. change the default 10% cutoff for the percentage of distinct values out of the total number of samples), you can use the `caret::preProcess()` function after running `preprocess_data` with `remove_nzv=FALSE` (see the `caret::nearZeroVar()` function for more information).

```{r}
# remove features with zero variance
preprocess_data(dataset = nonvar_df, outcome_colname = "outcome", remove_nzv = FALSE, remove_zv = TRUE)
```
...
## Next step: run machine learning!

After you preprocess your data (either using the `preprocess_data` function or by preprocessing the data on your own), you're ready to run machine learning! Please see the vignette on the `run_ml` function for more information about running machine learning. 
