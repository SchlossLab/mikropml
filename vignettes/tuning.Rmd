...
Here we run ML on 3 data splits with the new `lambda` range we defined above. 
...
As you can see, we get a mean maxima at 0.3 which is the best `lambda` value for this dataset when we run 3 data splits. This value is used for building the final model. For a better understanding of the global maxima, it would be better to run more datasplits. 
