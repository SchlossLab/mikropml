...
When we run `xgbTree` method, we are tuning the `nrounds`, `gamma`, `eta` `max_depth`, `colsample_bytree`, `min_child_weight` and `subsample` hyperparameters. Like other tree-based methods some of these hyperparameters need to be appropriate in relation to the number of features in the dataset.
