...
hparams_df <- dplyr::tibble(
  param = c("alpha", "lambda", "lambda", "lambda",  "lambda",  "lambda"),
  value = c(1, 10^-2, 0.015, 0.025, 0.05, 0.06),
  method = rep("glmnet", 6)
)
hparams_df
...
How do we know which `lambda` value is the best one? To answer that, we need to run the ML pipeline on multiple datasplits and look at the mean cross-validation performance of each `lambda` across those modeling experiments. We describe how to run ML with multiple datasplits here: http://www.schlosslab.org/mikropml/articles/parallel.html

Here we run ML on 3 dataplits with the new `lambda` range we defined above. We can then use `plot_hp_performance` to see which `lambda` gives us the largest mean AUC value across modeling experiments. 
...
As you can see, we get a maxima at 0.25 which is the best `lambda` value for this dataset. This value is used for building the final model.
...
When we run `rf` method, we are tuning the `mtry` hyperparameter. This is the number of features that are randomly collected to be sampled at each tree node. It needs to be an appropriate number in relation to the number of features in the dataset.
...
When we run `rpart2` method, we are tuning the `maxdepth` hyperparameter. This is the maximum depth of any node of the final tree.
...
When we run `svmRadial` method, we are tuning the `C` and `sigma` hyperparameters. 
`sigma` defines how far the influence of a single training example reaches and `C` behaves as a regularization parameter.

By default, we provide 2 separate ranges for the two. 
...
When we run `svmRadial` method, we are tuning the `nrounds`, `gamma`, `eta` `max_depth`, `colsample_bytree`, `min_child_weight` and `subsample` hyperparameters. Like other tree-based methods some of these hyperparameters need to be an appropriate in relation to the number of features in the dataset.

By default, we fix the `nrounds`, `gamma`, `colsample_bytree` and `min_child_weight` to set values and we provide a range of values for `eta`, `max_depth` and `subsample`. All of these can be changed and optimized by the user. 
