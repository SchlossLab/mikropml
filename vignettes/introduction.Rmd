...
We package `otu_mini` as a small example dataset with `mikropml`.
...
We will focus on `glmnet` which is our defualt implementation of L2-regularized logistic regression. here, then cover a few other examples towards the end.
...
When using logistic regression for binary classification, area under the receiver-operator characteristic curve (AUC) is a useful meric to evaluate model performance.
