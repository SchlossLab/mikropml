...
observations in the dataset are chosen for the training partition to satisfy the
`training_frac`. However, in some cases you might wish to control exactly which
observations are in the training partition. You can instead assign
`training_frac` a vector of indices that correspond to which rows of the dataset
should go in the training fraction of the train/test split.
...
train/test partition. This makes sense when you want to use groups to control
for batch effects. However, in some cases you might prefer to control exactly
which groups end up in which partition, and you might even be okay with some
observations from the same group being assigned to different partitions.

For example, say you want groups A and B to be used for training, C and D for
testing, and you don't have a preference for what happens to the other groups. You
can give the `group_partitions` parameter a named list to specify which groups
should go in the train partition and which should go in the test partition.
...
In another scenario, maybe you want only groups A and B to be used for training,
but you also want to allow other observations from A and Bto be used for
testing:
...
[setting custom training indices](#custom-training-indices) and/or providing
your own train control scheme with the `cross_val` parameter in `run_ml()`.
