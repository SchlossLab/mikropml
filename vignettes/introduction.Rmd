...
We've chosen a bunch of default arguments that are related to best practices [@topcuoglu_framework_2020]. 
However, we also allow you to change those arguments if you'd like. 
...
The input data to `run_ml()` is a dataframe where each row is a sample. 
One column (assumed to be the first) is the outcome of interest, 
and all of the other columns are the features. 
Here's an example that comes with `mikropml`:
...
feel free to open an issue on GitHub if you have questions about other models if you want to see if we can help.
...
```{r}
results <- run_ml(dataset = dat, 
                  method = 'glmnet', 
                  outcome_colname = 'dx', 
                  kfold = 3, 
                  seed = 2019) # once we make otu_mini larger, i'll get rid of kfold
```
...
`feature_importance` has information about feature importances if `find_feature_importance = TRUE` (the default is `FALSE`). 
