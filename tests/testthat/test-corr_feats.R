...
test_that("get_corr_feats works", {
  ...
  expect_equivalent(expect_warning(get_corr_feats(data.frame(
    ...
    c = c(0.6607978, 0.6291140, 0.06178627, 0.2059746)), 0.6),
    'deprecated'),
    cor_feats, tolerance = tol)
  expect_equivalent(expect_warning(get_corr_feats(data.frame(
...
test_that('correlation clustering helper functions work', {
  ...
  expect_equal(get_binary_corr_mat(features),
               corr_mat)
  null_mat <- matrix(numeric(0), nrow = 0, ncol = 0)
  expect_equal(get_binary_corr_mat(data.frame()),
               null_mat)

  expect_equal(cluster_corr_mat(corr_mat),
               c(a = 1L, b = 1L, c = 2L, d = 1L, e = 3L))
  expect_error(cluster_corr_mat(null_mat),
               "The correlation matrix contains nothing. Hint: is the features data frame empty?")

  expect_equal(get_groups_from_clusters(cluster_corr_mat(corr_mat)),
               c("a|b|d", "c", "e"))
})
