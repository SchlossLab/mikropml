predictions_sm <- c(2.044805e-03, 1.846221e-04, 4.331578e-03, 1.290319e-05,
                    9.939288e-01, 6.812802e-05, 9.983625e-01, 1.208421e-04,
                    4.403648e-03, 2.486293e-01, 1.853760e-05, 3.086007e-03)
predictions_med <- c(0.003330436, 0.935320188, 0.003146670, 0.050361065,
...
test_that("get_prediction works", {
    ...
    expect_equal(get_predictions(trained_model1, test_data1, 'cancer'),
                 predictions_med, tolerance = tol)
})
test_that("recode_outcome works", {
    expect_equal(recode_outcome(test_data_sm, 'dx', 'cancer'),
                 outcomes_sm)
    expect_equal(recode_outcome(test_data_sm, 'dx', 'normal'),
