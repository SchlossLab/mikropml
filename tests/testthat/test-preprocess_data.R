...
test_that("rm_corr_feats works", {
  expect_equal(rm_corr_feats(test_df[, c(2, 5, 8)]), list(
  ...
  expect_equal(
    rm_corr_feats(cbind(test_df[, c(2, 5, 8)], var8 = c(1, 1, 0))),
    list(
      features = structure(list(
        var1 = 1:3, var4 = c(0, 1, 0),
        grp1 = c(1, 1, 0)
  ...
  expect_equal(
    rm_corr_feats(dplyr::as_tibble(test_df[, c(2)])),
