...
test_that('correlated groups correct', {
  corr = dplyr::tibble(feature1 = c('A', 'C', 'D'),
                       feature2 = c('B', 'A', 'E'))
...
test_that('no correlated groups correct', {
  ...
  expect_equal(sort(group_correlated_features(corr, test_data)),
               c('A', 'B', 'C', 'D', 'E', 'F'))
})
...
test_that('permuted auc returns correct value for [fake] correlated feature',
          {
            fn_output = find_permuted_auc(trained_model_sm,
                                          test_data_sm,
                                          'dx',
                                          'Otu00049|Otu00050',
                                          'cancer')
...
feat_imps <- structure(list(auc = c(0.7955, 0.9065, 0.9045, 0.9, 0.9, 0.9,
                       ...
                       0.9, 0.9),
                       auc_diff = c(0.1045, -0.00649999999999999, -0.00449999999999999,
                       ...
                       names = structure(1:60, .Label = c("Otu00001",
                       ...
                       "Otu00056", "Otu00057", "Otu00058", "Otu00059", "Otu00060"), class = "factor")),
                       class = "data.frame", row.names = c(NA, -60L))
# get_feature_importance
test_that('feature importances are correct', {
  f_imps = get_feature_importance(train_data_sm,
                                  trained_model_sm,
                                  test_data_sm,
                                  'dx',
                                  'cancer')
